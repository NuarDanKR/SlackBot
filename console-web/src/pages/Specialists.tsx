...
export function SpecialistAnalytics({ query, navigate }: { query: URLSearchParams; navigate: (path: string) => void }) {
  ...
  if (res.loading) return <Loading what="봇 분류 상태를" />
  if (res.error || !res.data) return <Failed what="봇 분류 상태를" detail={res.error?.message ?? '응답이 없습니다.'} onRetry={res.reload} />
  ...
  const insufficient = calls.filter((c) => c.result === 'evidence_insufficient').length
  const avg = calls.length ? calls.reduce((sum, c) => sum + c.elapsedMs, 0) / calls.length : 0
  return <><PageHead crumb="답변 · 봇 분류 상태" title="봇 분류 상태" note="어떤 봇으로 분류됐는지 확인하고, 연결된 질문·답변 원본에서 그 판단의 실제 맥락을 검토합니다." />
    <Section title="호출 현황"><div className="metrics overview-metrics"><Metric k="호출" v={fmt.int(calls.length)} unit="건" /><Metric k="성공" v={fmt.int(success)} unit="건" /><Metric k="호출 실패" v={fmt.int(fallback + errors)} unit="건" /><Metric k="근거 부족" v={fmt.int(insufficient)} unit="건" /><Metric k="평균 응답" v={fmt.ms(avg)} /></div></Section>
    <Section title="호출 결과 분포" lead="성공, 호출 실패, 근거 부족과 계약 위반 건수를 비교합니다."><MiniBars label="전문 봇 호출 결과" items={[{ label: '성공', value: success, tone: 'ok' }, { label: '호출 실패', value: fallback + errors, tone: 'bad' }, { label: '근거 부족', value: insufficient, tone: 'warn' }, { label: '계약 위반', value: violations, tone: 'bad' }]} /></Section>
    <Section title="최근 호출"><form className="filter-row" onSubmit={(event) => { event.preventDefault(); navigate(withQuery('/answer/specialists', { specialist: specialistDraft.trim(), result: resultDraft })) }}>
      ...
      <div className="field"><label className="field-label" htmlFor="specialist-result">호출 결과</label><select id="specialist-result" className="input" value={resultDraft} onChange={(e) => setResultDraft(e.target.value)}><option value="">모든 결과</option><option value="success">성공</option><option value="fallback">폴백</option><option value="error">오류</option><option value="contract_violation">계약 위반</option><option value="evidence_insufficient">근거 부족</option></select></div>
      ...
      <div className="table-wrap"><table className="table"><thead><tr><th>시각</th><th>워크스페이스</th><th>전문 봇</th><th>작업</th><th>선택 이유</th><th>신뢰도</th><th>결과</th><th>질문·답변</th><th className="num">시간</th><th className="num">비용</th></tr></thead><tbody>{calls.map((call) => <tr key={call.id}><td>{fmt.dayClock(call.at)}</td><td>{call.workspace}</td><td className="mono">{call.specialist}</td><td><span className="mono">{call.taskKind || '-'}</span>{call.requiredCapability && <div className="hint mono">{call.requiredCapability}</div>}</td><td>{call.routingReason || '-'}</td><td>{call.confidence == null ? '-' : `${Math.round(call.confidence * 100)}%`}</td><td>{RESULT_LABEL[call.result]}{call.errorCode && <div className="hint mono">{call.errorCode}</div>}</td><td>{call.qaRecordKey ? <button className="btn btn-sm" type="button" onClick={() => navigate(withQuery('/answer/records', { record: call.qaRecordKey }))}>원본 보기</button> : <span className="hint">연결 전 기록</span>}</td><td className="num">{fmt.ms(call.elapsedMs)}</td><td className="num">{fmt.usd(call.costUsd)}</td></tr>)}</tbody></table></div>
    </Section></>
}
