...
export function SpecialistManagement({ user, query, onToast }: { user: ConsoleUser; query: URLSearchParams; onToast: (message: string) => void }) {
  ...
  return <><PageHead crumb="관리 · 전문 봇" title="전문 봇 등록·승인"
    note={user.role === 'admin'
      ? '전문 봇 개발자가 제출한 등록·변경 요청을 검토하고 승인하거나 반려합니다. 관리자는 전문 봇 소스를 직접 관리하지 않습니다.'
      : '전문 봇 개발자는 등록 정보와 변경 사항을 요청합니다. 적용 여부는 관리자가 검토하고 승인합니다.'} />
    ...
    {user.role === 'developer' && <Section title="변경 요청 등록" lead="전문 봇 개발자가 등록 정보와 변경 사항을 제출하는 화면입니다. 관리자가 승인한 뒤에만 적용되며, 활성화는 런타임 어댑터가 배포된 뒤에만 가능합니다."><div className="card card-pad"><div className="form-grid specialist-form">
