...
export function Questions({ query, navigate }: { query: URLSearchParams; navigate: (path: string) => void }) {
  ...
  return <><PageHead crumb="답변 · 질문 처리 기록" title="질문 처리 기록" note="질문 본문 없이 분류, 처리 결과, 근거 수, 모델과 소요 시간만 표시합니다." />
    <Section title="최근 처리" note={`${res.data.questions.length}건`}><form className="filter-row" onSubmit={(event) => { event.preventDefault(); navigate(withQuery('/answer/questions', { workspace: workspaceDraft.trim(), result: resultDraft })) }}>
        ...
        <div className="filter-actions"><button className="btn btn-primary btn-sm" type="submit">조회</button>{(workspace || result) && <button className="btn btn-sm btn-quiet" type="button" onClick={() => navigate('/answer/questions')}>초기화</button>}</div>
      </form>
