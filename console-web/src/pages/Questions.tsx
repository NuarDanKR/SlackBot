...
export function Questions({ query, navigate }: { query: URLSearchParams; navigate: (path: string) => void }) {
  ...
  return <><PageHead crumb="답변 · 질문 처리 기록" title="질문 처리 기록" note="질문 본문 없이 분류, 처리 결과, 근거 수, 모델과 소요 시간만 표시합니다." />
    <Section title="조회 기간" note={periodLabel(data.periodStart, data.periodEnd)}>
      <DateRangeFilter path="/answer/questions" query={query} navigate={navigate} defaultStart={data.periodStart} defaultEnd={data.periodEnd} today={data.today} preserve={{ workspace, result }} />
    </Section>
    <Section title="처리 기록" note={`${data.questions.length}건`}><form className="filter-row" onSubmit={(event) => { event.preventDefault(); navigate(withQuery('/answer/questions', { workspace: workspaceDraft.trim(), result: resultDraft, start: data.periodStart, end: data.periodEnd })) }}>
