...
좌측 아래 `담당자 화면 보기` 버튼으로 관리자(류대안)와 현업 담당자(김수현) 화면을 번갈아 볼 수
있습니다. 실제 운영에서는 로그인 사용자의 역할로 결정됩니다.

| | 관리자(owner) | 담당자(member) |
|---|---|---|
| 승인 버튼 | 보입니다 | 보이지 않습니다 (`승인 대기 중` 상태만) |
| 수집 문서 원문 | 확인 후 열람 가능 · 기록 남음 | 열리지 않습니다 (목록·상태만) |
| 워크스페이스 관리 | 보입니다 | 메뉴 자체가 없습니다 |
| 보이는 범위 | 전체 | 담당 워크스페이스만 |
