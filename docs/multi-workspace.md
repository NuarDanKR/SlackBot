...
# 상위(root) 워크스페이스 - 산하 자료를 전량 열람하고 채널 멤버십 필터를 받지 않는다.
...
### ③ root 워크스페이스 — 산하 자료 전량
경영본부처럼 취합·열람을 담당하는 조직은 `ROOT_WORKSPACES` 에 넣는다.
...
```
root 는:
- `CROSS_WS_READ` 대상 워크스페이스의 자료를 **문서 표시와 무관하게** 열람한다
  (`share_with` 를 일일이 지정하지 않아도 된다)
- **자기 워크스페이스 안에서 채널 멤버십 필터를 받지 않는다** — 소속되지 않은 채널도 조회 가능

`상태` 응답에 등급이 표시된다:
```
*워크스페이스*: 경영본부 (`mgmt`) · 등급: 상위(root) - 산하 자료 전량 열람
*크로스 열람 허용*: pilot
```
...
- root 는 산하 private 자료까지 열람, 자기 워크스페이스 멤버십 면제
- 화이트리스트는 단방향
