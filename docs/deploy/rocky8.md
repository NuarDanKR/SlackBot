...
`수신대상=0` 이면 사번↔Slack 매핑이 없다 — 설정 문제가 아니다.
...
과 `specialist_runtime_schema.sql` 은 **한 번도 적용되지 않았다.**

그래서 콘솔 「봇 분류 상태」 가 `column "qa_record_id" does not exist` 로 죽었다
(2026-09-14). 대조해 보니 빠진 것이 컬럼 9개와 표 4개였다 — 나머지는 그 화면을 아직
안 열어서 안 터졌을 뿐이었다.

전부 멱등하다. 배포할 때마다 돌려도 된다. 순서는 스크립트가 안다(뒤 파일이 앞 파일의
표를 외래키로 참조한다).

적용됐는지는 대조로 확인한다 — 스크립트가 끝에 자동으로 돌린다:

```bash
cd /opt/tybot && sudo -u tybot .venv/bin/python scripts/check_schema_drift.py
```

`✅ 선언과 실제가 같다` 가 나와야 한다. 빠진 것이 있으면 그 표·컬럼과 어느 파일에
적혀 있는지를 찍는다. **화면이 죽기 전에 여기서 드러난다.**
...
**사용자 설정도 기본 수신으로 바꿨다**(2026-09-11). 소속 부서에 열린 폴더의 일정은
그룹웨어에서 이미 볼 수 있는 것이므로, 따로 켜게 하면 기능이 있는데 수신자가 0명인
상태가 오래 간다. 그래서 `/일정 알림` 은 **끄거나 분을 바꾸는** 수단이다.

- **10분 전 고정.** 분을 고르는 버튼은 없다 — 알림은 오거나 안 오거나다
- 설정 행이 없으면 그대로 받는다. `/일정 알림` 은 **끄는 스위치**다
- 본인이 끈 사람은 계속 안 받는다 — 기본값이 그 결정을 되살리지 않는다
- 여러 워크스페이스에 있으면 **가장 최근에 연결한 곳** 한 군데로만 간다

예전에 30분·둘 다를 고른 사람이 있으면 한 번 맞춰 준다(멱등):

```bash
sudo cat /opt/tybot/deploy/sql/schedule_dm_fixed_ten.sql | sudo -u postgres psql -p 55432 -d tyslackai -f -
```

### 받는 사람은 사번이 이어진 재직자뿐이다

`user_identity` 가 유일한 관문이다. 그런데 이 매핑은 **사람이 봇을 쓸 때 만들어진다**
(`identity.ensure`). 그래서 봇을 써 볼 이유가 없던 사람은 이메일이 맞아도 빠진다 —
8명 팀에서 4명만 잡혀 있었다(2026-09-11).

봇이 기동할 때마다 워크스페이스 멤버를 훑어 이어 준다(`identity_sweep`). 재시작을
기다리지 않고 지금 잇고 싶으면:

```bash
cd /opt/tybot
sudo -u tybot .venv/bin/python scripts/backfill_identity.py --dry-run   # 무엇이 바뀔지
sudo -u tybot .venv/bin/python scripts/backfill_identity.py             # 실제로
```

안 이어진 사람이 목록으로 나온다. 원인은 둘뿐이다 — Slack 프로필에 이메일이 없거나,
그 이메일이 `employee.email` 과 다르다. 외부·게스트 계정은 안 이어지는 것이 맞다. 더해지는 것은 밀어 주기뿐이다.
