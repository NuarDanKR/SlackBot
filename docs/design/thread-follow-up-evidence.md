...
# 2026-09-14 보완: 답변 형식 변경

동일 스레드에서 이전 답변을 bullet point 등으로 바꾸라는 요청은 `format_only`로
표시하고 직전 근거 범위를 유지한다. planner의 전문 능력, 선택 후보, 신뢰도는
후속 요청 병합 시 보존한다.

이전 답변 본문은 `editing_text`로만 전달한다. 현재 권한으로 원문 참조를 전부
복원했을 때만 전문 봇의 편집 대상으로 사용할 수 있다. 일부 참조가 사라지거나
권한이 없으면 편집을 중단한다. 일반 검색의 근거와 아카이브에는 넣지 않는다.
형식 편집은 전문 봇 모델을 검색 도구 없이 한 번 호출하고 출처는 마스터가 다시 붙인다.

도구 탐색 예산은 45초이며 외부 전문 봇 호출 제한은 90초로 조정한다.
실행 중인 네트워크 요청을 스레드 취소만으로 중단하지 못하는 기존 제약은 남아 있다.
실 Slack 주간보고 답변의 재서식과 운영 응답시간은 배포 후 재검증한다.
