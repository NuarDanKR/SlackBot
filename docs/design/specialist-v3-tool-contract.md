...
## 10. 열린 질문 — 2026-09-23 정리

넷 중 셋은 **Hermes 를 고치지 않고** 정해진다. 우리는 Hermes 를 수정하지 않는다 —
PF 가 commit 하면 받아서 쓴다.

### 1. 예산 기본값 — **우리 쪽**, ENV 로 뺀다
집행하는 것은 우리 `ToolBox.ToolBudget` 이다. 계약이 숫자를 보내는 것은 Hermes 가
자기 쪽에서도 세어 헛호출을 줄이게 하려는 것이고, **판정은 우리가 한다.**

그래서 Hermes 수정이 아니라 **우리 쪽 변수화**가 맞다. 값을 코드에 박으면 PF 관찰값이
나올 때마다 배포해야 한다.

PF 에 필요한 것은 하나뿐이다 — **질문당 실제 검색·읽기 횟수 관찰값.**

### 2. `fetch_recent_slack` — **주지 않는다**
분리 결정 §4.C 가 「방금 올린 것」 문제를 **수집 완료 ack** 로 푼다. Hermes 에
Slack 경로를 또 주면 수집 주인이 둘이 되고, 그건 이번 분리가 없애려던 것이다.

`allow` 기본값에서 뺀다. Hermes 는 Archiving Bot 이 확정한 것만 본다.

### 3. 부분 답변 — **우리가 계약에서 정한다**: `answered` + `uncertain`
`failed` 가 아니다. 우리 `ToolBox` 는 예산이 끝나면 이미 이렇게 돌려준다.
...
읽은 것이 있는데 `failed` 로 버리면 사람은 답을 못 받고, 마스터 폴백이 같은 자료를
다시 찾는다. 대신 **어디까지 찾았는지를 `uncertain` 에 적는다** — 그게 없으면 부분
답변이 완전한 답으로 보인다.

Hermes 쪽 구현은 v3 를 만드는 일의 일부이고, 별도 수정 요청이 아니다.

### 4. 비용 귀속 — **이미 해결됐다**
Hermes 인스턴스가 둘이다(오너 결정 2026-09-23).
...
각 인스턴스가 **자기 env 의 자기 키**를 쓰므로 비용이 저절로 갈린다. 모드 분리
PR(수정 요청 §2)의 「독립된 env/credential」 에 이미 들어 있어 추가 요청이 없다.

`usage.cost_usd` 는 보고용이다 — 우리 인스턴스 비용은 우리가 보고, 상한은 TY DMZ
env 에서 건다.

## 10.1 PF 에 요청할 것

우리가 Hermes 코드를 고치지 않으므로, 필요한 것은 **값을 밖에서 주입할 수 있게**
해 달라는 요청뿐이다.

1. **질문당 검색·읽기 횟수 관찰값** — 예산 기본값을 정하려면 필요하다
2. **하드코딩된 상한을 env/config 로** — 검색 횟수, 읽기 크기, timeout, 모델 이름.
   인스턴스가 둘이고 부하가 다르므로, 값이 코드에 있으면 한쪽에 맞추면 다른 쪽이
   틀린다
3. **모델 키·게이트웨이 주소를 env 에서** — 비용이 갈리는 지점이다(§10.4)

값을 바꾸려고 PF 소스를 우리가 고치면 다음 `git pull` 에 지워진다. 그게 「Hermes 를
수정하지 않는다」 가 실무에서 뜻하는 것이다.
