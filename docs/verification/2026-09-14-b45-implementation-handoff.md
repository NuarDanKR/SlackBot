...
- 오너 추가 요청: 명시 요청만 Canvas였던 정책을 변경. 이 기준은 2026-09-16에 다시
  조정되어 **출처 블록을 제외한 본문 25줄까지는 메시지, 26줄부터 자동 Canvas**다.
  글자 수만으로 Canvas를 강제하지 않으며 `메시지로` 요청하면 자동 전환하지 않는다.
  일반 TYBot/Hermes 본문은 3,000자 이내 압축 요약을 기본으로 한다.
