...
## 프로젝트는 조직이 아니다
프로젝트는 정식 조직이 아니라 조직코드가 없다. 그래서 조직도(`org_unit`)에도 없고
검색에도 나오지 않는다. 채널명 규칙은 코드를 요구하므로 **주관 조직의 코드를 빌린다** —
`프로젝트-스마트시티_ABB110-회의`. 사용자가 구분에서 프로젝트를 직접 고르고 프로젝트명을
적으면, 조직 칸에서 고른 주관 조직의 코드가 들어간다. 그래야 소속 제한·권한 상속이
주관 조직 기준으로 그대로 이어진다(별도 코드 체계를 만들면 그 연결이 끊긴다).
...
def derive_prefix(code: str, name: str) -> str:
    ...
    순서가 중요하다:
    1. 이름이 `프로젝트` 로 끝나면 프로젝트다(수동 입력 경로용. 조직도에는 프로젝트가 없다).
