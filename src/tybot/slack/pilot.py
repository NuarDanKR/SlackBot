...
# 「이런 자료도 근거로 읽느냐」 에 대한 **결정적** 답. LLM 을 부르지 않는다.
#
# 자료 종류마다 문장을 따로 두면 반드시 샌다 — 캔버스만 적어 두었더니 "그럼
# 채널에 있는 폴더는?" 이 전체 사용법으로 빠졌다(2026-09-14). 읽는 것과
# **안 읽는 것**을 한자리에 적는다.
#
# 안 읽는 것을 적는 이유가 더 크다. "못 찾았습니다" 만 나가면 사용자는 자료가
# 없다고 읽는데, 실제로는 우리가 그 자리를 보지 않는 것이다. 둘은 사람이 할
# 일이 완전히 다르다 — 하나는 자료를 올리는 것이고 하나는 우리에게 말하는 것이다.
SOURCE_SCOPE_ANSWER = (
    "*근거로 읽는 것*\n"
    "• 채널에 쌓인 **대화 원문** — 사람이 쓴 메시지와 스레드 답글\n"
    "• **첨부 본문** — 봇이 본 메시지에 올라온 파일을 수집 시점에 자동 변환한 것 "
    "(PDF·HWP·Office·이미지 OCR). 변환에 실패했으면 실패했다고 밝힙니다\n"
    "• **채널 Canvas 본문** — 현재 내용과 수집된 스냅샷. 다만 Canvas 안에 "
    "걸어 둔 파일과 링크는 따라가지 않습니다 — 이름만 보이고 내용은 없습니다\n"
    "• **최근 실시간 대화** — 아직 아카이브에 안 들어온 오늘치. 답변에 `[실시간]` 으로 구분합니다\n"
    "\n*읽지 않는 것*\n"
    "• **채널 파일 목록(폴더)** — 파일 탭에 모아 둔 목록은 조회하지 않습니다. "
    "봇이 **초대되기 전**에 올라온 파일이나 수집 범위 밖의 파일은 아예 모릅니다. "
    "그 파일을 쓰려면 채널에 다시 올리거나 `수집` 으로 과거 대화를 백필하세요\n"
    "• **Canvas 안에 걸린 첨부 파일과 링크 대상** — 본문 글자만 읽습니다\n"
    "• 외부 링크 안의 내용, 다른 워크스페이스 자료(화이트리스트 제외)\n"
    "• TYBot 이 만든 답변 Canvas — 답을 다시 근거로 쓰지 않습니다\n"
    "\n*범위*\n"
    "본인이 접근 가능한 채널만 조회합니다. DM 에서는 현재 채널이 없으므로 "
    "채널 이름을 함께 적어 주세요. 조회 권한이나 다운로드가 실패하면 읽었다고 "
    "가장하지 않고 근거를 확보하지 못했다고 답합니다."
)
