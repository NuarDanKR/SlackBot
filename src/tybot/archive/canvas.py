...
def canvas_file_id(client, channel_id: str) -> str | None:
    ...
    except Exception as e:  # noqa: BLE001 - 캔버스 없음은 정상 상황이다
        logger.debug("conversations.info 실패 %s: %s", channel_id, e)
        return None
...
def _to_lines(raw: bytes, mimetype: str) -> list[str]:
    ...
    if "html" in mimetype or head.startswith(("<!doctype html", "<html")):
        p = _TextExtractor()
        p.feed(text)
        p.close()
        lines = p.parts
...
def canvas_lines(client, channel_id: str, bot_token: str | None) -> tuple[list[str], list[str]]:
    """채널 캔버스 → (원문에 넣을 줄들, 경고들).
    ...
    if not file_id:
        return [], []
    if not bot_token:
        return [f"[캔버스:미변환] {file_id}"], ["캔버스: 토큰이 없어 본문을 가져오지 못했습니다"]

    try:
        info = client.files_info(file=file_id)
    except Exception as e:  # noqa: BLE001
        return [f"[캔버스:미변환] {file_id}"], [f"캔버스 {file_id}: files.info 실패 - {e}"]
    ...
    try:
        ...
        return [f"[캔버스:미변환] {title}"], [f"캔버스 {title}: {e}"]
