...
# 신고 처리 약속. **운영 규칙과 같아야 한다** — 화면에서 약속하고 지키지 않으면
# 신고 창구 자체가 신뢰를 잃는다. 규칙을 바꾸면 이 문구도 함께 바꾼다.
HANDLING_PROMISE = (
    "신고는 담당자가 *영업일 기준 2일 안에* 확인하고, 원인이 확인되면 회귀 테스트로 "
    "만들어 같은 오류가 다시 나지 않게 합니다. 확인 결과는 이 대화로 회신드립니다."
)
...
def thanks(kind: str, *, linked: str = "") -> str:
    ...
    # 신고 뒤에 무슨 일이 일어나는지 말하지 않으면 사람은 한 번 신고하고 끝낸다.
    # 사내 피드백의 "오류 시 어떻게 대응해야 하는지" 가 정확히 이 지점이었다.
    lines.append(HANDLING_PROMISE)
