...
변환 실패에는 실패 사유와 Slack 원본 링크를 붙인다. OCR·PII 검사를 통과한 이미지만
권한 있는 사용자의 관련 질문에서 시각 분석에 쓰며, 실패 원본을 보내는 승인 통로는 없다.
"""
    ...
    out: list[dict] = [
        {
            ...
            "text": {
                ...
                "text": (
                    f"*첨부 처리 현황{where}*\n"
                    "문서 변환은 자동으로 진행됩니다. OCR·PII 검사를 통과한 이미지는 "
                    "관련 질문의 시각 분석에 쓰이고, 실패한 원본은 전송하지 않습니다."
                ),
            },
        }
    ]
