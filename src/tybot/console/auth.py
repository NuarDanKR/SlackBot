...
# TODO(B-20): 파일럿 편의를 위한 임시 계정. **운영 전에 반드시 제거한다.**
# 계정을 PostgreSQL 로 옮긴 뒤(B-16), 계정 설정이 비면 로그인을 열지 말고 기동을 멈추게 바꾼다
# — 아무도 못 들어가는 편이 아무나 들어오는 것보다 낫다.
# 지금은 기본 계정이 쓰이면 기동 로그 경고와 화면 배너가 뜬다(아래 Authenticator 참조).
