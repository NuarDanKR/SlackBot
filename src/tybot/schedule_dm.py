...
# --- 개인 자격 ---------------------------------------------------------------
#
# 알림을 켠 사람에게 "켜졌습니다" 만 말하면, 받을 폴더가 없어도 켜진 것처럼 보인다.
# 그 사람은 회의 30분 전에 오지 않는 알림을 기다리고, 안 오면 봇을 안 믿는다.
#
# 자격의 근거는 **그룹웨어 폴더 ACL** 이다(2026-09-11). 소속 부서에 열려 있으면 받는다 —
# 우리가 따로 승인하지 않는다. 그래서 0이라는 것은 "승인 대기" 가 아니라 "그룹웨어에서
# 그 부서에 열린 폴더가 없다" 는 뜻이고, 조치할 곳도 그룹웨어다.
#
...
NOT_ENTITLED = (
    "⚠️ 설정은 저장했지만 *지금은 알림이 오지 않습니다.* 그룹웨어에서 회원님 부서에"
    " 열려 있는 팀 일정 폴더가 없습니다.\n"
    "이 설정을 다시 켤 필요는 없습니다 — 그룹웨어 쪽 폴더 권한이 생기면 그때부터"
    " 자동으로 옵니다."
)
