...
새 워크스페이스 `$1` 을 Hermes Hub에 온보딩한다. 다음을 순서대로 수행/확인:

1. **앱 설치** — 해당 워크스페이스에 Hermes 앱 설치, `xoxb-`/`xapp-` 발급. (토큰은 서버 시크릿 매니저에만, 저장소 금지)
2. **채널 명명 검증** — `channel-naming` 스킬로 채널명 규칙 점검.
3. **봇 초대** — 대상 채널 전부에 `/invite @Hermes`. 초대 누락 목록을 출력.
