...
수집→요약해 **중앙 MD 아카이브**에 쌓고, 모든 봇은 권한이 허용된 **중앙 아카이브의 원문**
(과 아직 수집 전인 실시간 Slack 대화)을 근거로 답한다. 멀티 LLM(Claude·GPT 등)을 게이트웨이로 라우팅하며 모델 선택을 지원한다.

## 절대 원칙 (위반 시 데이터 자산 오염 — 리뷰에서 반려)
1. **원문 보존·요약 재귀 금지** — 봇 답변/요약을 아카이브에 되먹이지 않는다. 원문만 저장.
   실시간 조회로 읽은 대화에도 같다 — **봇 발언은 싣지 않는다.**
   후속 질문도 같다 — 이어 가는 것은 이전 답변 문장이 아니라 그 답변이 읽은 **원문
   좌표**(`EvidenceRef`)이고, 매번 현재 권한으로 다시 연다.
...
2. **출처 강제** — 모든 답변에 `출처: #채널, 📄문서(날짜)`. 출처 없으면 근거 없는 답.
   아직 아카이브에 없는 실시간 대화는 **Slack 메시지 링크**를 출처로 붙인다.
   아카이브 문서로 붙이면 그 문서에 없는 내용이 되어 사람이 확인하러 갔다가 못 찾는다.
...
> **2026-09-11 (오너 결정) — 근거 범위를 「아카이브」 에서 「아카이브 + 실시간」 으로.**
>
> 사람이 봇과 **이어서 묻기** 때문이다. 방금 오간 이야기가 검색에 안 잡히면
> "모른다" 만 답하게 되고, 그건 대화가 아니다.
>
> 넓힌 것은 **근거의 범위뿐**이다. 나머지는 그대로다.
> - 권한은 아카이브와 같은 기준 — 요청자가 볼 수 있는 채널만
>   (`specialist_tools.ToolBox._fetch_recent`)
> - 봇 발언 제외(원칙 1), Slack 링크 출처 + `[실시간]` 표시(원칙 2)
