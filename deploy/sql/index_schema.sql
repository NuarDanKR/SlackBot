...
BEGIN
    IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'tyslackai') THEN
        EXECUTE 'GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE org_unit, employee, user_identity, sync_run, channel, raw_line, audit_query TO tyslackai';
        ...
        FOR seq_name IN
            SELECT quote_ident(n.nspname) || '.' || quote_ident(s.relname)
             ...
             WHERE s.relkind = 'S' AND t.relname = ANY(ARRAY['org_unit', 'employee', 'user_identity', 'sync_run', 'channel', 'raw_line', 'audit_query'])
        LOOP
