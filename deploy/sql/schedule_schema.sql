...
BEGIN
    IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'tyslackai') THEN
        EXECUTE 'GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE schedule_folder, schedule_channel, schedule_sync_run, schedule_occurrence, schedule_delivery TO tyslackai';
        ...
        FOR seq_name IN
            SELECT quote_ident(n.nspname) || '.' || quote_ident(s.relname)
             ...
             WHERE s.relkind = 'S' AND t.relname = ANY(ARRAY['schedule_folder', 'schedule_channel', 'schedule_sync_run', 'schedule_occurrence', 'schedule_delivery'])
        LOOP
